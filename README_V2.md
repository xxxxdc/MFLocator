...
项目主要的文件结构如下所示。代码部分包含我们的方法以及作为baseline的VCMATCH方法。
...
├── gitrepo/						# 代码存储库文件夹，存放项目相关的十个代码存储库
├── pretrained_model/				# 预训练模型参数文件夹，含roberta-large和bert
│   ├── roberta-large/
│   ├── BERT/
├── README_V2.md
```
...
2. 克隆代码存储库。尝试运行以下代码，借助GitPython库克隆实验过程中需要使用的十个代码存储库。
...
3. 将[roberta-large](https://huggingface.co/FacebookAI/roberta-large/tree/main)模型的相关文件下载至pretrained_model/roberta-large目录下。

4. 依次按照collect_data.py, create_dataset.py, get_tokens.py, process_data.py, feature.py, ranking.py的顺序运行文件。

5. 在ranking.py指定的result_path文件夹中查看性能指标文件。
